...
const INIT = {
  ...
  ccCashbackRate: 3,
  checkingBalance: 0,
  checking2Balance: 0,       // Community Choice Credit Union
  ccBalance: 0,
  k401AccountBalance: 0,     // current 401k account balance
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <Card style={{ borderColor: "#339AF033" }}>
          ...
          <div style={{ color: "#339AF0", fontSize: 20, fontWeight: 800, fontFamily: "monospace", marginTop: 4 }}>
            {fmtFull(state.checkingBalance)}
          </div>
          <div style={{ color: "#64748B", fontSize: 11, marginTop: 6 }}>Primary</div>
        </Card>
        <Card style={{ borderColor: "#20C99733" }}>
          <div style={{ color: "#64748B", fontSize: 11, fontWeight: 700, letterSpacing: 1 }}>CCCU</div>
          <div style={{ color: "#20C997", fontSize: 20, fontWeight: 800, fontFamily: "monospace", marginTop: 4 }}>
            {fmtFull(state.checking2Balance || 0)}
          </div>
          <div style={{ color: "#64748B", fontSize: 11, marginTop: 6 }}>Community Choice</div>
        </Card>
        <Card style={{ borderColor: "#845EF733" }}>
          <div style={{ color: "#64748B", fontSize: 11, fontWeight: 700, letterSpacing: 1 }}>PAYPAL CC</div>
          <div style={{ color: "#845EF7", fontSize: 20, fontWeight: 800, fontFamily: "monospace", marginTop: 4 }}>
        ...
        <Card style={{ borderColor: "#845EF733" }}>
          <div style={{ color: "#64748B", fontSize: 11, fontWeight: 700, letterSpacing: 1 }}>401(k)</div>
          <div style={{ color: "#845EF7", fontSize: 20, fontWeight: 800, fontFamily: "monospace", marginTop: 4 }}>
            {fmtFull(state.k401AccountBalance || 0)}
          </div>
          <div style={{ color: "#64748B", fontSize: 11, marginTop: 6 }}>Account balance</div>
        </Card>
      </div>
...
function CheckingTab({ state, setState }) {
  ...
  const [bal2, setBal2] = useState(state.checking2Balance || 0);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <Card style={{ borderColor: "#339AF033" }}>
        ...
        <div style={{ color: "#339AF0", fontSize: 32, fontWeight: 800, fontFamily: "monospace", margin: "6px 0" }}>
          {fmtFull(state.checkingBalance)}
        </div>
      ...
      <Card style={{ borderColor: "#20C99733" }}>
        ...
        <div style={{ color: "#20C997", fontSize: 32, fontWeight: 800, fontFamily: "monospace", margin: "6px 0" }}>
          {fmtFull(state.checking2Balance || 0)}
        </div>
        <button onClick={() => { setBal2(state.checking2Balance || 0); setShowEdit2(true); }} style={{
          background: "#20C99722", border: "1px solid #20C99744", color: "#20C997",
          borderRadius: 8, padding: "8px 16px", cursor: "pointer", fontSize: 13, fontWeight: 600
        }}>Update Balance</button>
      </Card>
      ...
      {showEdit && (
        <Modal title="Update Primary Checking" icon="🏦" color="#339AF0" onClose={() => setShowEdit(false)}>
          <Label>BALANCE ($)</Label>
      ...
      {showEdit2 && (
        <Modal title="Update Community Choice CU" icon="🏦" color="#20C997" onClose={() => setShowEdit2(false)}>
          <Label>BALANCE ($)</Label>
          <input autoFocus type="number" value={bal2} onChange={e => setBal2(e.target.value)}
            style={inputStyle({ fontSize: 22, fontWeight: 700 })} />
          <button onClick={() => {
            setState(s => ({ ...s, checking2Balance: +bal2 }));
            setShowEdit2(false);
          }} style={{
            marginTop: 16, width: "100%", background: "#20C997", border: "none",
            color: "#0A0E1A", borderRadius: 8, padding: 12, cursor: "pointer",
            fontSize: 14, fontWeight: 700
          }}>Save</button>
        </Modal>
      ...
      {modal && (() => {
        ...
        return (
          <Modal title={`Add ${cat.label} Income`} icon={cat.icon} color={cat.color} onClose={() => setModal(null)}>
            ...
            <Label>AMOUNT ($)</Label>
            <input autoFocus type="text" placeholder="0" value={amount}
              onChange={e => setAmount(e.target.value.replace(/[^0-9.]/g, ""))} style={inputStyle({ fontSize: 22, fontWeight: 700 })} />
            <Label>NOTE (optional)</Label>
...
function ExpensesTab({ state, setState }) {
  ...
  const save = () => {
    ...
    const actualPayFrom = isRent ? "checking" : payFrom;
    setState(s => ({
      ...s,
      expenseEntries: [...s.expenseEntries, {
        id: Date.now(), category: modal, type: "expense",
        amount: clean, note, month: +month, year, payFrom: actualPayFrom
      }],
      ccBalance: actualPayFrom === "cc" ? s.ccBalance + clean : s.ccBalance,
      checkingBalance: actualPayFrom === "checking" ? s.checkingBalance - clean : s.checkingBalance,
      checking2Balance: actualPayFrom === "checking2" ? s.checking2Balance - clean : s.checking2Balance,
    }));
    setModal(null); setAmount(""); setNote(""); setPayFrom("cc");
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                ...
                <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                  ...
                  <button onClick={() => setState(s => {
                    ...
                    return {
                      ...
                      ccBalance: pf === "cc" ? Math.max(0, s.ccBalance - e.amount) : s.ccBalance,
                      checkingBalance: pf === "checking" ? s.checkingBalance + e.amount : s.checkingBalance,
                      checking2Balance: pf === "checking2" ? s.checking2Balance + e.amount : s.checking2Balance,
                    };
                  })} style={{ background: "transparent", border: "none", color: "#EF4444", cursor: "pointer", fontSize: 14 }}>×</button>
                </div>
              </div>
      ...
      {modal && (() => {
        ...
        const isRent = modal === "housing";
        return (
          <Modal title={`Add ${cat.label}`} icon={cat.icon} color={cat.color} onClose={() => { setModal(null); setPayFrom("cc"); }}>
            <Label>MONTH</Label>
            <select value={month} onChange={e => setMonth(+e.target.value)} style={inputStyle()}>
              {MONTHS.map((m, i) => <option key={i} value={i}>{m} {year}</option>)}
            </select>
            <Label>AMOUNT ($)</Label>
            <input autoFocus type="text" placeholder="0" value={amount}
              onChange={e => setAmount(formatAmt(e.target.value))} style={inputStyle({ fontSize: 22, fontWeight: 700 })} />
            <Label>NOTE (optional)</Label>
            ...
            {isRent ? (
              ...
              <>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 4 }}>
                  {[
                    { val: "cc", label: "💳 PayPal CC" },
                    { val: "checking", label: "🏦 Checking" },
                    { val: "checking2", label: "🏦 CCCU" },
                  ].map(opt => (
                    <button key={opt.val} onClick={() => setPayFrom(opt.val)} style={{
                      flex: 1, background: payFrom === opt.val ? cat.color + "33" : "#0F1320",
                      border: `1px solid ${payFrom === opt.val ? cat.color : "#2D3548"}`,
                      color: payFrom === opt.val ? cat.color : "#64748B",
                      borderRadius: 8, padding: "8px 4px", cursor: "pointer", fontSize: 11, fontWeight: 600
                    }}>{opt.label}</button>
                  ))}
                </div>
              </>
            ...
            <button onClick={save} style={{
              marginTop: 16, width: "100%", background: cat.color, border: "none",
              color: "#fff", borderRadius: 8, padding: 12, cursor: "pointer", fontSize: 14, fontWeight: 700
            }}>Save · {isRent ? "Deduct from Checking" : payFrom === "cc" ? "Add to CC Balance" : payFrom === "checking2" ? "Deduct from CCCU" : "Deduct from Checking"}</button>
          </Modal>
        );
      })()}
    </div>
...
function K401Tab({ state, setState }) {
  ...
  const [newBal, setNewBal] = useState(state.k401AccountBalance || 0);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      ...
      <Card style={{ borderColor: "#845EF733" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            ...
            <div style={{ color: "#845EF7", fontSize: 30, fontWeight: 800, fontFamily: "monospace", marginTop: 4 }}>
              {fmtFull(state.k401AccountBalance || 0)}
            </div>
            <div style={{ color: "#64748B", fontSize: 11, marginTop: 4 }}>Update when you check your statement</div>
          </div>
          <button onClick={() => { setNewBal(state.k401AccountBalance || 0); setShowBalEdit(true); }} style={{
            background: "#845EF722", border: "1px solid #845EF744", color: "#845EF7",
      ...
      {showBalEdit && (
        <Modal title="Update 401(k) Balance" icon="🏦" color="#845EF7" onClose={() => setShowBalEdit(false)}>
          <Label>CURRENT ACCOUNT BALANCE ($)</Label>
          <input autoFocus type="number" placeholder="0" value={newBal}
            onChange={e => setNewBal(e.target.value)} style={inputStyle({ fontSize: 22, fontWeight: 700 })} />
          <div style={{ color: "#64748B", fontSize: 12, marginTop: 8 }}>
            Enter the balance shown on your Fidelity statement
          </div>
          <button onClick={() => {
            setState(s => ({ ...s, k401AccountBalance: +newBal }));
